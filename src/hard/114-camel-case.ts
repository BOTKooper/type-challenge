...
type Camelize<S extends string> = S extends `${infer F}_${infer R}`
  ? `${Capitalize<F>}${Camelize<R>}`
  : Capitalize<S>;

type CamelCase<S extends string> = Lowercase<S> extends `${infer F}_${infer R}`
  ? `${F}${Camelize<R>}`
  : Lowercase<S>;
